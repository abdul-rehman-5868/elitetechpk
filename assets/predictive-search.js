...
class PredictiveSearchComponent extends Component {
  ...
  get #isModalSearch() {
    return Boolean(this.closest('.search-modal'));
  }
  ...
  connectedCallback() {
    ...
    if (!this.#isModalSearch && RecentlyViewed.getProducts().length > 0) {
      requestIdleCallback(() => {
        this.#loadEmptyState();
      });
    }
    ...
    const containers = Array.from(
      this.querySelectorAll(
        '.predictive-search-results__suggestions, ' +
          ...
          '.predictive-search-resource-list__items, ' +
          '.search-modal__default-links, ' +
    ...
    const allItems = containers
      .flatMap((container) => {
        if (
          ...
          container.classList.contains('predictive-search-results__rows')
        ) {
          return Array.from(container.querySelectorAll('.predictive-search-results__row, .predictive-search-results__card'));
        }
        return Array.from(container.querySelectorAll('[ref="resultsItems[]"], .predictive-search-results__card, .predictive-search-results__suggestion'));
      })
  ...
  #resetScrollPositions() {
    requestAnimationFrame(() => {
      ...
      this.querySelector('.search-modal__results .predictive-search-results__inner')?.scrollTo(0, 0);
  ...
  #resetSearch = async () => {
    ...
    const isModalSearch = this.#isModalSearch;
    const emptySectionId = isModalSearch ? 'predictive-search-modal-empty' : 'predictive-search-empty';
    ...
    const parsedEmptySectionMarkup = new DOMParser()
      ...
      .querySelector('.predictive-search-modal-empty-section, .predictive-search-empty-section');
    ...
    if (isModalSearch && !searchInput.value) {
      document.dispatchEvent(new CustomEvent('search-modal:finish-typewriter'));
    }
  };
}
