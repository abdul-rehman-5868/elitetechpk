    ...
    scheduler.schedule(() => {
      ...
      requestAnimationFrame(() => {
        if (this.initialSlideIndex !== 0 && initialSlideId) {
          ...
          // select() no-ops when the requested slide is already `current` (true here,
          // since current defaults to initialSlideIndex above), so scroll-snapped
          // layouts (e.g. scroll-snap-align: center with scroll-padding, as in the
          // peek carousel) never get their initial snap applied and the first slide
          // sits at raw scrollLeft: 0 instead of its snapped position. Scroll to it
          // directly so slides with symmetric peek start centered like every other slide.
          const firstSlide = this.refs.slides?.[0];
          if (firstSlide) this.#scroll?.to(firstSlide, { instant: true });
