...
/**
 * A drawer that opens from the right side by default.
 *
 * On wide viewports (≥ 990px) drawers without `[overlay]` squeeze page content
 * alongside them via a non-modal dialog (`show()`). Drawers with `[overlay]`
 * always use modal overlay mode (`showModal()`), never squeezing the page.
 *
 * On narrow viewports (< 990px) every drawer overlays with a backdrop.
 *
 * Set `data-side="start"` to slide in from the left instead of the right.
 *
...
export class ThemeDrawer extends Component {
  ...
  /**
   * @returns {boolean} Whether this drawer always overlays (never squeezes).
   */
  get #isOverlay() {
  ...
  /**
   * @returns {boolean} Whether this drawer should use modal dialog semantics.
   */
  get #useModal() {
  ...
  /**
   * @returns {boolean} Whether the drawer opens from the inline-start (left) edge.
   */
  get #opensFromStart() {
    return this.dataset.side === 'start';
  }

  /**
   * @param {'inline-start' | undefined} enter
   * @returns {string}
   */
  #openingClass(enter) {
    if (this.#opensFromStart || enter === 'inline-start') {
      return 'theme-drawer__dialog--opening-start';
    }
    return 'theme-drawer__dialog--opening';
  }
   ...
   */
  #closingClass() {
    return this.#opensFromStart ? 'theme-drawer__dialog--closing-start' : 'theme-drawer__dialog--closing';
  }
  ...
  open({ enter } = {}) {
    ...
    const openClass = this.#openingClass(enter);
  ...
  async #closeDialog() {
    ...
    if (panel.open) {
      ...
      panel.classList.remove(
        ...
        'theme-drawer__dialog--opening-start',
        'theme-drawer__dialog--closing',
        'theme-drawer__dialog--closing-start'
      );

      const closeClass = this.#closingClass();
