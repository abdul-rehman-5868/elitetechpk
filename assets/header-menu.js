...
class HeaderMenu extends Component {
  ...
  connectedCallback() {
    ...
    this.#bindMegaMenuPointerHandlers();
  }
  ...
  disconnectedCallback() {
    ...
    this.#megaMenuAbortController?.abort();
    this.#megaMenuAbortController = null;
  }

  /** @type {AbortController | null} */
  #megaMenuAbortController = null;

  #bindMegaMenuPointerHandlers() {
    this.#megaMenuAbortController?.abort();
    this.#megaMenuAbortController = new AbortController();
    const { signal } = this.#megaMenuAbortController;

    this.querySelectorAll('.menu-list__list-item--shop, .menu-list__list-item--collections').forEach((listItem) => {
      if (!(listItem instanceof HTMLElement)) return;

      const submenu = listItem.querySelector('[ref="submenu[]"]');
      ...
      submenu.addEventListener(
        ...
        (event) => {
          ...
          if (this.#state.activeItem && listItem.contains(this.#state.activeItem)) return;
      ...
      submenu.addEventListener(
        ...
        (event) => {
          ...
          requestAnimationFrame(() => {
            const overSubmenu = this.#isPointerOverActiveSubmenu();
            const listItemHovered = listItem.matches(':hover');

            if (overSubmenu) return;
            if (listItemHovered) return;
            this.#scheduleDeferredDeactivate();
          });
        },
        { signal }
      );
  ...
  /** Bumps when activate cancels in-flight deferred deactivates. */
  #deactivateGeneration = 0;

  #cancelPendingDeactivate() {
    this.#deactivateGeneration += 1;
  }

  /**
   * @param {number} generation
   */
  #scheduleDeferredDeactivate(generation = this.#deactivateGeneration) {
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        if (generation !== this.#deactivateGeneration) return;
        if (this.#isPointerOverActiveSubmenu()) return;
        if (this.#isPointerOverSiblingMegaNavItem()) return;
        this.#deactivate();
      });
    });
  }
   ...
   * @returns {boolean}
   */
  #isMovingToSiblingMegaNavItem(relatedTarget) {
    if (!(relatedTarget instanceof Element)) return false;

    const siblingItem = relatedTarget.closest('.menu-list__list-item--shop, .menu-list__list-item--collections');
    if (!(siblingItem instanceof HTMLElement)) return false;

    const activeListItem = this.#state.activeItem?.closest('.menu-list__list-item');
    return Boolean(activeListItem && siblingItem !== activeListItem);
  }

  /**
   * @returns {boolean}
   */
  #isPointerOverSiblingMegaNavItem() {
    const { x, y } = this.#lastPointer;
    const hit = document.elementFromPoint(x, y);
    if (!(hit instanceof Element)) return false;

    const hitItem = hit.closest('.menu-list__list-item--shop, .menu-list__list-item--collections');
    if (!(hitItem instanceof HTMLElement)) return false;

    const activeListItem = this.#state.activeItem?.closest('.menu-list__list-item');
    return Boolean(activeListItem && hitItem !== activeListItem);
  }
  ...
  activate = (event) => {
    ...
    if (submenu) {
        ...
        requestAnimationFrame(() => {
          ...
          requestAnimationFrame(() => {
            if (submenu.offsetHeight > 0) {
              ...
              this.#setFullOpenHeaderHeight(submenu.offsetHeight, headerVisibleHeight);
              this.#cleanupMutationObserver();
            }
          });
        });
      ...
      // Re-measure after layout in case overflow/content-visibility delayed the first paint
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          if (!this.#state.activeItem || !submenu.isConnected) return;
          const nextHeight = submenu.offsetHeight || 0;
          if (nextHeight <= 0) return;
          const headerVisibleHeight = this.#getHeaderVisibleHeight();
          this.headerComponent?.style.setProperty('--submenu-height', `${nextHeight}px`);
          this.#setFullOpenHeaderHeight(nextHeight, headerVisibleHeight);
        });
      });
  ...
  /**
   * Whether the pointer is over the active item's submenu (including abs-positioned panels).
   * @returns {boolean}
   */
  #isPointerOverActiveSubmenu() {
    ...
    const { x, y } = this.#lastPointer;
    const probeOffsets = [0, 4, 8, 12, 20, 32, 48, 64];

    for (const offsetY of probeOffsets) {
      const hit = document.elementFromPoint(x, y + offsetY);
      if (hit instanceof Node && submenu.contains(hit)) return true;
    }
  ...
  deactivate(event) {
    ...
    const submenu = findSubmenu(this.#state.activeItem);
    const isMovingWithinMenu = event.relatedTarget instanceof Node && submenu?.contains(document.activeElement);
    const isMovingToSubmenu =
      event.relatedTarget instanceof Node &&
      (submenu?.contains(event.relatedTarget) ||
        (event.relatedTarget instanceof Element &&
          Boolean(event.relatedTarget.closest('.shop-mega-menu, .collections-mega-menu'))));
    const isMovingToOverflowMenu =
      event.relatedTarget instanceof Element && Boolean(event.relatedTarget.closest('[slot="overflow"]'));
    const isMovingToSiblingMega = this.#isMovingToSiblingMegaNavItem(event.relatedTarget);

    if (isMovingWithinMenu || isMovingToOverflowMenu || isMovingToSubmenu || isMovingToSiblingMega) {
      if (isMovingToSiblingMega) {
        this.#cancelPendingDeactivate();
      }
      if (this.#state.activeItem) {
        this.#stopPointerTracking(this.#state.activeItem);
    ...
    if (submenu && event.type === 'pointerleave') {
      this.#scheduleDeferredDeactivate();
      return;
    }
  ...
  #deactivate = (item = this.#state.activeItem) => {
    ...
    const submenuHovered = submenu instanceof HTMLElement && submenu.matches(':hover');
    const pointerOverSubmenu = this.#isPointerOverActiveSubmenu();

    if (submenu instanceof HTMLElement && (submenuHovered || pointerOverSubmenu)) {
      return;
    }
