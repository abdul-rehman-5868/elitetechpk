...
class PriceFacetComponent extends Component {
  ...
  #bindRangeInputs() {
    ...
    minRange.addEventListener('input', this.#onRangeInput);
    maxRange.addEventListener('input', this.#onRangeInput);
  }
  ...
  #onRangeInput = (event) => {
    ...
    if (event.target === minRange && minVal > maxVal) {
      minVal = maxVal;
      minRange.value = String(minVal);
    }
    if (event.target === maxRange && maxVal < minVal) {
      maxVal = minVal;
  ...
  #syncRangeProgress() {
    const { minRange, maxRange, rangeProgress } = this.refs;
    if (!(minRange instanceof HTMLInputElement) || !(maxRange instanceof HTMLInputElement)) return;

    const max = Number(minRange.max) || 1;
    const minVal = Number(minRange.value) || 0;
    const maxVal = Number(maxRange.value) || max;
    const start = Math.max(0, Math.min(100, (minVal / max) * 100));
    const end = Math.max(0, Math.min(100, (maxVal / max) * 100));

    if (rangeProgress instanceof HTMLElement) {
      rangeProgress.style.setProperty('--range-start', `${start}%`);
      rangeProgress.style.setProperty('--range-end', `${end}%`);
    }
  }
  ...
  updatePriceFilterAndResults() {
    ...
    if (minRange instanceof HTMLInputElement && maxRange instanceof HTMLInputElement) {
      const minDisplay = Number.parseFloat(String(minInput.value || '0').replace(/,/g, ''));
      const maxDisplay = Number.parseFloat(String(maxInput.value || maxRange.max || '0').replace(/,/g, ''));
      if (!Number.isNaN(minDisplay)) minRange.value = String(minDisplay);
      if (!Number.isNaN(maxDisplay)) maxRange.value = String(maxDisplay);
      this.#syncRangeProgress();
    }
...
class SortingFilterComponent extends Component {
  ...
  updateFilterAndSorting(event) {
    ...
    // Keep the drawer status label in sync before the section morphs
    if (event.target instanceof HTMLInputElement && event.target.name === 'sort_by') {
      const statusEl = this.querySelector('.facets__status, .sorting-filter__status-value');
      if (statusEl && event.target.dataset.optionName) {
        statusEl.textContent = event.target.dataset.optionName;
      }
    }
